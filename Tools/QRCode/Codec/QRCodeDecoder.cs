...
using QRCodeImage = Ophelia.Tools.QRCode.Codec.Data.IQRCodeImage;
using QRCodeSymbol = Ophelia.Tools.QRCode.Codec.Data.QRCodeSymbol;
using ReedSolomon = Ophelia.Tools.QRCode.Codec.Ecc.ReedSolomon;
using DecodingFailedException = Ophelia.Tools.QRCode.Exceptions.DecodingFailedException;
using InvalidDataBlockException = Ophelia.Tools.QRCode.Exceptions.InvalidDataBlockException;
...
using QRCodeDataBlockReader = Ophelia.Tools.QRCode.Codec.Reader.QRCodeDataBlockReader;
using QRCodeImageReader = Ophelia.Tools.QRCode.Codec.Reader.QRCodeImageReader;
using DebugCanvas = Ophelia.Tools.QRCode.Codec.Util.IDebugCanvas;
using DebugCanvasAdapter = Ophelia.Tools.QRCode.Codec.Util.DebugCanvasAdapter;
	...
	public class QRCodeDecoder
	{
		...
		public virtual sbyte[] DecodeBytes(QRCodeImage qrCodeImage)
		{
			...
			for (int i = 0; i < results.Count; i++)
			{
				...
				if (result.ErrorNumber < lowestError)
				{
                    lowestError = result.ErrorNumber;
					lowestErrorIndex = i;
				}
			}
		...
		internal virtual int[] CorrectDataBlocks(int[] blocks)
		{
			...
			if (numRSBlocks == 1)
			{
				...
				corrector.Correct();
				numCorrections += corrector.CorrectedErrorsNumber;
				if (numCorrections > 0)
