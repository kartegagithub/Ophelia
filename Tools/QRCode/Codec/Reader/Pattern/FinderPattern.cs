...
using InvalidVersionInfoException = Ophelia.Tools.QRCode.Exceptions.InvalidVersionInfoException;
...
namespace Ophelia.Tools.QRCode.Codec.Reader.Pattern
{
	...
	public class FinderPattern
	{
		...
		public static FinderPattern FindFinderPattern(bool[][] image)
		{
			...
			int version = CalculateRoughVersion(center, width);
			if (version > 6)
			{
				try
				{
					version = CalculateExactVersion(center, sincos, moduleSize, image);
				}
		...
		internal static Line[] FindLineAcross(bool[][] image)
		{
			...
			while (true)
			{
				...
				if (direction == READ_HORIZONTAL)
				{
					...
					{
						current.SetRenamed(0, current.Y + 1);
						lengthBuffer = new int[5];
					}
					else
					{
						current.SetRenamed(0, 0); //reset target point
						lengthBuffer = new int[5];
				...
				else
				{
					...
					{
						current.SetRenamed(current.X + 1, 0);
						lengthBuffer = new int[5];
					}
					else
					{
			...
			canvas.DrawLines(foundLines, Ophelia.Tools.QRCode.Codec.Util.ColorCode.LIGHTGREEN);
			return foundLines;
		}
		...
		internal static bool CantNeighbor(Line line1, Line line2)
		{
			...
			if (line1.Horizontal)
			{
				if (System.Math.Abs(line1.GetFirstPoint().Y - line2.GetFirstPoint().Y) > 1)
					return true;
				else
					return false;
			}
			...
			{
				if (System.Math.Abs(line1.GetFirstPoint().X - line2.GetFirstPoint().X) > 1)
					return true;
				else
					return false;
			}
		...
		internal static int[] GetAngle(Point[] centers)
		{
			...
			Line remoteLine = Line.GetLongestLine(additionalLine);
			Point originPoint = new Point();
			for (int i = 0; i < centers.Length; i++)
			{
				if (!remoteLine.GetFirstPoint().Equals(centers[i]) && !remoteLine.GetSecondPoint().Equals(centers[i]))
				{
					originPoint = centers[i];
					break;
				}
			}
			canvas.Println("Origin Point is: " + originPoint);
			...
			if (originPoint.Y <= remoteLine.GetFirstPoint().Y & originPoint.Y <= remoteLine.GetSecondPoint().Y)
				if (remoteLine.GetFirstPoint().X < remoteLine.GetSecondPoint().X)
					remotePoint = remoteLine.GetSecondPoint();
				else
					remotePoint = remoteLine.GetFirstPoint();
			else if (originPoint.X >= remoteLine.GetFirstPoint().X & originPoint.X >= remoteLine.GetSecondPoint().X)
				if (remoteLine.GetFirstPoint().Y < remoteLine.GetSecondPoint().Y)
					remotePoint = remoteLine.GetSecondPoint();
				else
					remotePoint = remoteLine.GetFirstPoint();
			else if (originPoint.Y >= remoteLine.GetFirstPoint().Y & originPoint.Y >= remoteLine.GetSecondPoint().Y)
				if (remoteLine.GetFirstPoint().X < remoteLine.GetSecondPoint().X)
					remotePoint = remoteLine.GetFirstPoint();
				else
					remotePoint = remoteLine.GetSecondPoint();
			//1st or 4th quadrant
			else if (remoteLine.GetFirstPoint().Y < remoteLine.GetSecondPoint().Y)
				remotePoint = remoteLine.GetFirstPoint();
			else
				remotePoint = remoteLine.GetSecondPoint();
		...
		internal static Point[] GetCenter(Line[] crossLines)
		{
			...
			{
				canvas.DrawPolygon(foundPoints, Ophelia.Tools.QRCode.Codec.Util.ColorCode.RED);
				return foundPoints;
			}
			else
		...
		internal static int CalculateRoughVersion(Point[] center, int[] width)
		{
		...
		internal static int CalculateExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
		{
			...
			canvas.DrawPoints(points, Ophelia.Tools.QRCode.Codec.Util.ColorCode.RED);
