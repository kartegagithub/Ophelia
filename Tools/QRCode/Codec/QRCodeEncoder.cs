...
using Ophelia.Tools.QRCode.Properties;
...
namespace Ophelia.Tools.QRCode.Codec
{
    public class QRCodeEncoder
    {
        internal ErrorCorrection qrCodeErrorCorrect;
        internal EncodeMode qrCodeEncodeMode;
        ...
        public QRCodeEncoder()
        {
            ...
            qrCodeEncodeMode = EncodeMode.BYTE;
            qrCodeVersion = 7;

            qrCodeStructureAppendN = 0;
            qrCodeStructureAppendM = 0;
            qrCodeStructureAppendParity = 0;
            qrCodeStructureAppendOriginalData = string.Empty;
        ...
        virtual public ErrorCorrection QRCodeErrorCorrect
        {
            get
            {
        ...
        virtual public EncodeMode QRCodeEncodeMode
        {
            get
            {
        ...
        public virtual bool[][] CalculateQrcode(byte[] qrcodeData)
        {
            ...
            try
            {
                ...
                using (MemoryStream memoryStream = new MemoryStream(Resources.GetResource(fileName)))
                {
                    using (BufferedStream bufferedStream = new BufferedStream(memoryStream))
                    {

                        SystemUtils.ReadInput(bufferedStream, matrixX, 0, matrixX.Length);
                        SystemUtils.ReadInput(bufferedStream, matrixY, 0, matrixY.Length);
                        SystemUtils.ReadInput(bufferedStream, maskArray, 0, maskArray.Length);
                        SystemUtils.ReadInput(bufferedStream, formatInformationX2, 0, formatInformationX2.Length);
                        SystemUtils.ReadInput(bufferedStream, formatInformationY2, 0, formatInformationY2.Length);
                        SystemUtils.ReadInput(bufferedStream, rsEccCodewords, 0, rsEccCodewords.Length);
                        SystemUtils.ReadInput(bufferedStream, rsBlockOrderTemp, 0, rsBlockOrderTemp.Length);
            ...
            try
            {
                ...
                using (MemoryStream memoryStream = new MemoryStream(Resources.GetResource(fileName)))
                {
                    using (BufferedStream bufferedStream = new BufferedStream(memoryStream))
                    {
                        SystemUtils.ReadInput(bufferedStream, frameData, 0, frameData.Length);
                        bufferedStream.Close();
                    }
                    memoryStream.Close();
                }
            ...
            bool[][] outRenamed = new bool[modules1Side][];
            for (int i3 = 0; i3 < modules1Side; i3++)
            {
                outRenamed[i3] = new bool[modules1Side];
            }
