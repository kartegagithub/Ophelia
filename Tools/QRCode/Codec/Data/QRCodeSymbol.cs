...
using BCH15_5 = Ophelia.Tools.QRCode.Codec.Ecc.BCH15_5;
using LogicalSeed = Ophelia.Tools.QRCode.Codec.Reader.Pattern.LogicalSeed;
...
namespace Ophelia.Tools.QRCode.Codec.Data
{
	public class QRCodeSymbol
	{
		...
		virtual public int NumErrorCollectionCode
		{
			get
			{
				return this.numErrorCollectionCode[version - 1][this.errorCollectionLevel];
			}
		...
		virtual public int NumRSBlocks
		{
			get
			{
				return this.numRSBlocks[version - 1][this.errorCollectionLevel];
			}
		...
		internal virtual void  Initialize()
		{
			...
			dataCapacity = CalculateDataCapacity();			
		...
		private int CalculateDataCapacity()
		{
			int nFunctionPatternModule = 0;
			int nFormatAndVersionInfoModule = 0;
			int version = this.Version;
			
			if (version <= 6)
				nFormatAndVersionInfoModule = 31;
			else
				nFormatAndVersionInfoModule = 67;
			...
			nFunctionPatternModule = modulesLeft + 8 * version + 2 - (sqrtCenters - 2) * 10;			
			int dataCapacity = (width * width - nFunctionPatternModule - nFormatAndVersionInfoModule) / 8;
			return dataCapacity;
		}
		...
		{
			this.moduleMatrix[x][y] = !moduleMatrix[x][y];
		}
