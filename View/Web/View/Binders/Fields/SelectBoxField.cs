...
{
	public class SelectBoxField : Field
	{
		...
		public new View.Web.Controls.SelectBox Control {
		...
		internal override void OnBeforeDraw()
		{
			...
			if (!string.IsNullOrEmpty(this.NewLinkUrl) || !string.IsNullOrEmpty(this.EditLinkUrl)) {
				...
				if (!string.IsNullOrEmpty(this.EditLinkUrl)) {
					...
					string Url = "";
					if (!string.IsNullOrEmpty(this.EditLinkMemberName) && this.EditLinkUrl.IndexOf(this.EditLinkMemberName) > -1) {
						Url = this.EditLinkUrl.Insert(this.EditLinkUrl.IndexOf(this.EditLinkMemberName) + this.EditLinkMemberName.Length + 1, "$$");
					} else if (this.EditLinkUrl.IndexOf("?") > -1) {
		...
		{
			base.CreateControls();
			this.oControl = new View.Web.Controls.SelectBox(this.MemberName);
		}
		public SelectBoxField(ref FieldCollection Collection, string Header, string MemberName) : base(Collection, Header, MemberName)
		{
		}
	}
}
